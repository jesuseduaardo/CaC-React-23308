 ...
 */
const recomedador = () => {
    const menu = ['Peliculas', 'Series', 'Libros', 'Salir'];
    const peliculas = [
        "Avatar (2009)",
        "Titanic (1997)",
    ...
    let mensaje = '¿Que le gustaria ver?';
    for (let i = 0; i < menu.length; i++) {
        mensaje += `\n ${(i + 1)}.-${menu[i]}`;
    }
    let seleccionado;
    do {
        seleccionado = prompt(mensaje)
    } while (isNaN(seleccionado) || seleccionado > 4 || seleccionado < 1);
    let recomendacion;
    let indice;
    switch (seleccionado) {
        case "1": //Peliculas
            indice = Math.floor(Math.random() * peliculas.length);
            recomendacion = "ver " + peliculas[indice];
            break;
        case "2": //Series
            indice = Math.floor(Math.random() * series.length);
            recomendacion = "ver " + series[indice];
            break;
        case "3": //Libros
            indice = Math.floor(Math.random() * libros.length);
            recomendacion = "leer " + libros[indice];
            break;
        case "4":
            alert("Hasta luego");
            return;
    }
    alert("Te recomiendo " + recomendacion);
}
...
function deEuroAdolar(euro) {
    ...
    //Escribe tu código aquí
    const dolares = (euro * 1.20).toFixed(2);
...
function esVocal(letra) {
    ...
    if (letra.lenght > 1 || vocales.indexOf(letra.toLowerCase()) === -1) {
